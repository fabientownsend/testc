...
/*
  ...
  Example using the Grove Temperature and Humidity sensor UPM library.
  Takes a temperature and humidity reading every three seconds, before displaying them via serial communication.
...
float ModTemp(char modifier, float modifiedTemp){
  if (modifier == 'F') {
    modifiedTemp = ConvertCelciusToFahrenheit(modifiedTemp);
  } else if (modifier == 'K') {
    modifiedTemp = ConvertCelciusToKelvin(modifiedTemp);
  }
